...
{
    TestStringUtils::copy_memory_call_count++;

    if (!TestStringUtils::copy_memory_failure_calls.empty()) {
        if (TestStringUtils::copy_memory_call_count-1 >= TestStringUtils::copy_memory_failure_calls.size()) {
            throw std::runtime_error("copy_memory_failure_calls does not have enough items, call count: " + std::to_string(TestStringUtils::copy_memory_call_count));
        }

        if (TestStringUtils::copy_memory_failure_calls[TestStringUtils::copy_memory_call_count-1]) {
            return -1;
        }
    }
...
TEST_F(TestStringUtils, TestCopyMemoryFails) {
    TestStringUtils::set_copy_memory_failure_calls({true});

    auto result = pybytes_to_bytes_and_size(PyBytes_FromString("test"), nullptr);

    ASSERT_EQ(TestStringUtils::copy_memory_call_count, 1);
    ASSERT_EQ(result, nullptr);
}

TEST_F(TestStringUtils, TestSecondCopyMemoryFails) {
    TestStringUtils::set_copy_memory_failure_calls({false, true});

    Py_ssize_t size = -1;
    auto result = pybytes_to_bytes_and_size(PyBytes_FromString("test"), &size);

    ASSERT_EQ(TestStringUtils::copy_memory_call_count, 2);
    ASSERT_EQ(result, nullptr);
}

TEST_F(TestStringUtils, TestPyBytesToBytesAndSizeSimple) {
...
TEST_F(TestStringUtils, TestPyBytesToBytesAndSizeBinary) {
...
TEST_F(TestStringUtils, TestPyBytesToBytesAndSizeLarge) {
...
TEST_F(TestStringUtils, TestPyUnicodeToUtf8Empty) {
...
TEST_F(TestStringUtils, TestPyUnicodeToUtf8CopyTypeFails) {
...
TEST_F(TestStringUtils, TestPyUnicodeToUtf8CopyGenericFails) {
...
TEST_F(TestStringUtils, TestPyUnicodeToUtf8ExceedsMaxLength) {
...
TEST_F(TestStringUtils, TestPyUnicodeToUtf8NonASCIIMultiByte) {
