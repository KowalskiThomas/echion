...
public:
    ...
    [[nodiscard]] static Result<std::unique_ptr<ThreadInfo>> create(uintptr_t thread_id, unsigned long native_id, const char* name)
    {
        ...
        auto update_cpu_time_success = thread_info->update_cpu_time();
...
[[nodiscard]] inline Result<void> ThreadInfo::unwind(PyThreadState* tstate)
{
    ...
    {
        auto unwind_python_stack_success = unwind_python_stack(tstate);
        if (!unwind_python_stack_success)
            return Result<void>::error(ErrorKind::UnwindError);

        if (asyncio_loop)
        {
            (void)unwind_tasks();
            // If we failed to unwind tasks, keep going.
        }
        ...
        auto unwind_greenlets_success = unwind_greenlets(tstate, native_id);
        if (!unwind_greenlets_success) {
            return Result<void>::error(ErrorKind::UnwindError);
        }
    }
...
// ----------------------------------------------------------------------------
inline Result<void> ThreadInfo::unwind_greenlets(PyThreadState* tstate, unsigned long native_id)
{
    ...
    for (auto& greenlet_info : greenlet_info_map)
    {
        ...
        auto unwind_success = greenlet->unwind(frame, tstate, stack);
        if (!unwind_success)
            return Result<void>::error(ErrorKind::UnwindError);
        ...
        for (;;)
        {
            ...
            auto unwind_success = parent_greenlet->second->unwind(parent_frame, tstate, stack);
            if (!unwind_success)
                return Result<void>::error(ErrorKind::UnwindError);
...
inline Result<void> ThreadInfo::sample(int64_t iid, PyThreadState* tstate, microsecond_t delta)
{
    ...
    auto unwind_success = unwind(tstate);
    if (!unwind_success) {
        return Result<void>::error(ErrorKind::UnwindError);
    }
        ...
        if (local_greenlets.empty())
        {
            ...
            if (native)
            {
                ...
                if (!interleave_success)
                {
                    ...
                    return Result<void>::error(interleave_success.error());
                }
    ...
    else
    {
        for (auto& task_stack_info : local_tasks)
        {
            ...
            if (native)
            {
                ...
                // Python < 3.11.
                auto interleave_success = interleave_stacks(task_stack_info->stack);
                if (!interleave_success)
    ...
    if (!local_greenlets.empty())
    {
        for (auto& greenlet_stack : local_greenlets)
        {
            ...
            if (native)
            {
                // NOTE: These stacks might be non-sensical, especially with
                // Python < 3.11.
                auto interleave_success = interleave_stacks(stack);
                if (!interleave_success) {
                    return Result<void>::error(ErrorKind::StackChunkError);
                }
