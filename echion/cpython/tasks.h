...
inline PyObject* PyGen_yf(PyGenObject* gen, PyObject* frame_addr)
{
    ...
    if (gen->gi_frame_state < FRAME_CLEARED)
    {
        ...
        {
            std::cerr << indent << "  " << "localsplus copy failed, cancel" << std::endl;
            return NULL;
        }
        ...
        std::cerr << indent << "  " << "frame.f_code: " << frame.f_code << std::endl;
        std::cerr << indent << "  " << "frame.f_func: " << frame.f_func << std::endl;
        for (ssize_t i = 0; i < frame.stacktop; i++) {
            ...
            // auto maybe_str = PyObject_Str(addr);
            auto maybe_str = (PyObject*)(nullptr);
            if (maybe_str) {
                auto unicode = pyunicode_to_utf8(maybe_str);
                if (unicode) {
                    std::cerr << indent << "  " << "localsplus[" << i << "]: " << addr << " (" << *unicode << ")" << std::endl;
                } else {
                    std::cerr << indent << "  " << "localsplus[" << i << "]: " << addr << " (failed to convert to unicode)" << std::endl;
                }
            } else {
                std::cerr << indent << "  " << "localsplus[" << i << "]: " << addr << " (failed to convert to str)" << std::endl;
            }
        }
