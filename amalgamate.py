...
def generate_file(out_file: str):
    print(f"Generating file {out_file}")
    ...
    print(f"Found {len(cc_files)} source files")
    print("Header dependency order:")
    for i, h_file in enumerate(h_files):
        rel_path = os.path.relpath(h_file, ECHION_DIR)
        deps = dependencies.get(rel_path, [])
        print(f"  {i+1:2d}. {rel_path} (depends on: {', '.join(deps) if deps else 'none'})")
...
if __name__ == "__main__":
    generate_file(os.path.join(REPO_ROOT, "echion.cpp"))